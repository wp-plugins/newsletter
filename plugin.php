...
// Sends out all the notifications for the post "$post_id" and skipping the "$email" because it's the email
// of the author
function newsletter_send($subject, $message, $recipients = null)
{
    ...
    if ($recipients == null)
        $recipients = $wpdb->get_results("select * from " . $wpdb->prefix . "newsletter where status='C'");
        
    if (!$recipients) 
    {
        newsletter_log('Nessuna sottoscrizione');
        return;
    }
...
function newsletter_search($text)
{
    ...
    $recipients = $wpdb->get_results("select * from " . $wpdb->prefix . "newsletter where email like '%" .
        $wpdb->escape($text) . "%' order by email");
    if (!$recipients) return null;
    return $recipients;
}
